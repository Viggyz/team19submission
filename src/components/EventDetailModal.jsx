...
function EventDetailModal({
      ...
      Promise.allSettled(promises)
        .then(([eventPromise, userPromise]) => {
          ...
          if (userPromise.status === "fulfilled") {
            if (
              eventPromise.value.data.interested.some(
                (acc) => acc.id === userPromise.value.data.id
              )
            )
              setInterest(true);
            else {
  ...
  return (
    <Modal open={openEventDetailModal} onClose={handleEventDetailModalClose}>
      <Paper
        style={{
          position: "absolute",
          top: "50%",
      ...
      >
        ...
        <Typography>
          {moment(eventDetails.start_time).format("Mo MMM hh:mm A")} -{" "}
          {moment(eventDetails.end_time).format("Mo MMM hh:mm A")}
        </Typography>
        <Typography variant="body1">
          Created By: {eventDetails.created_by.username}
        </Typography>
        <Chip
          variant="filled"
          color="info"
          label={`${eventDetails.max_people} People`}
          sx={{ mx: "0.5rem" }}
        />
        <Divider sx={{ my: "1em" }} />
        ...
        >
          ...
          >
            ...
            <Typography variant="subtitle1">
              +91 {eventDetails.created_by.contact_no}
            </Typography>
          </Box>
          <Box
            sx={{
